...
export class AddmaterialsComponent implements OnInit {
    ...
    this.bookForm = this.formBuilder.group({
      ...
      authors: this.formBuilder.array([
        this.formBuilder.group({
          authorName: ['', [Validators.maxLength(40)]],
        }),
      ]),
      copyright: [this.currentYear],
  ...
  validBookImage = false;
  validPeriodicalImage = false;
  validAVImage = false;
  bookImgChangeEvt: any = '';
  periodicalImgChangeEvt: any = '';
  AVImgChangeEvt: any = '';
  bookCropImagePreview: SafeUrl | undefined;
  periodicalCropImagePreview: SafeUrl | undefined;
  AVCropImagePreview: SafeUrl | undefined;
    ...
    if (input.files && input.files.length) {
      ...
      if (file.type.startsWith('image/')) {
        if (type == 'book') {
          ...
          this.bookImgChangeEvt = event;
          this.bookCropImagePreview = '';
        } else if (type == 'periodical') {
          this.validPeriodicalImage = true;
          this.periodicalImgChangeEvt = event;
          this.periodicalCropImagePreview = '';
        } else if (type == 'AV') {
          this.validAVImage = true;
          this.AVImgChangeEvt = event;
          this.AVCropImagePreview = '';
        }
  ...
  cropImg(event: ImageCroppedEvent, type: string) {
    if (event?.objectUrl) {
      if (type == 'book') {
        this.bookCropImagePreview = this.sanitizer.bypassSecurityTrustUrl(
          event.objectUrl,
        );
      } else if (type == 'periodical') {
        this.periodicalCropImagePreview = this.sanitizer.bypassSecurityTrustUrl(
          event.objectUrl,
        );
      } else if (type == 'AV') {
        this.AVCropImagePreview = this.sanitizer.bypassSecurityTrustUrl(
          event.objectUrl,
        );
      }
    ...
    if (addForm.valid) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          Object.entries(addForm.value).forEach(
            ([key, value]: [string, any]) => {
              if (key == 'authors') {
                let authors = [];
                for (
                  let i = 0;
                  i < (addForm.get('authors') as FormArray).length;
                  i++
                ) {
                  authors.push(
                    (
                      (addForm.get('authors') as FormArray).at(i) as FormGroup
                    ).get('authorName')?.value,
                  );
                }
                form.append('authors', JSON.stringify(authors));
              } else if (value != '' && value != null) form.append(key, value);
            },
          );
          ...
          if (type == 'book') {
            ...
            this.ds
              .request('POST', 'materials/periodicals/process', form)
              .subscribe({
                next: (res: any) => {
                  this.successMessage(formTitle);
                },
            ...
            this.ds
              .request('POST', 'materials/articles/process', form)
              .subscribe({
                next: (res: any) => {
                  this.successMessage(formTitle);
                },
            ...
            this.ds
              .request('POST', 'materials/audio-visuals/process', form)
              .subscribe({
                next: (res: any) => {
                  this.successMessage(formTitle);
                },
  ...
  serverErrors(err: string) {
    if (err.toLowerCase().includes('sqlstate[23000]'))
      var text = 'Duplicate accession number.';
  ...
  importmaterialBtnClick() {
    ...
    modal.afterClosed().subscribe((result: { success: any }) => {
