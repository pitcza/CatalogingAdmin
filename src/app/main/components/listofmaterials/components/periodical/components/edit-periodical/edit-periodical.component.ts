...
export class EditPeriodicalComponent implements OnInit {
  ...
  constructor(
    private ref: MatDialogRef<EditPeriodicalComponent>,
  ...
  ngOnInit(): void {
    this.ds
      .request('GET', 'material/id/' + this.data.details, null)
      .subscribe((res: any) => {
        this.periodical = res;
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.ds
          .request(
            'DELETE',
            'materials/archive/' + this.periodical.accession,
            null
          )
          .subscribe({
            next: (res: any) => {
        ...
        ([key, value]: [string, any]) => {
          if (key == 'authors') {
            ...
            for (
              ...
              authors.push(
                (
                  (this.editForm.get('authors') as FormArray).at(i) as FormGroup
                ).get('authorName')?.value
              );
            }
            form.append('authors', JSON.stringify(authors));
          } else if (value || key == 'periodical_type') form.append(key, value);
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          this.ds
            .request(
              'PUT',
              'materials/periodicals/process/' + this.data.details,
              form
            )
            .subscribe({
              next: (res: any) => {
